DROP FUNCTION IF EXISTS public.match_sourceable_products_hybrid(vector, vector, double precision, integer, double precision, double precision, double precision, text[]);

CREATE FUNCTION public.match_sourceable_products_hybrid(
...
RETURNS TABLE(
  ...
  used_signals text[],
  normalized_query_tokens text[],
  synonym_expanded_tokens text[],
  product_attr_tokens text[]
)
LANGUAGE sql
...
AS $function$
  WITH
  stopwords AS (
    SELECT ARRAY['the','and','for','with','from','your','our','this','that','are','was','were','any','all','new','old','top']::text[] AS words
  ),
  tokenize_q AS (
    SELECT CASE
      ...
      ELSE ARRAY(
        SELECT DISTINCT t
        FROM (
          SELECT btrim(lower(tok)) AS t
          FROM unnest(query_attribute_keywords) k
          CROSS JOIN LATERAL unnest(regexp_split_to_array(k, '[\s\-,/]+')) tok
          WHERE k IS NOT NULL
        ) s, stopwords sw
        WHERE t IS NOT NULL
          AND t <> ''
          AND length(t) >= 3
          AND NOT (t = ANY(sw.words))
        ORDER BY t
      )
    END AS qtok_raw
  ),
    ...
    SELECT sp.id,
      ARRAY(
        ...
        FROM (
          ...
          CROSS JOIN LATERAL unnest(regexp_split_to_array(a, '[\s\-,/]+')) tok
          WHERE a IS NOT NULL
          UNION
          SELECT CASE btrim(lower(tok))
            WHEN 'trousers' THEN 'pants'
            WHEN 'pant' THEN 'pants'
            WHEN 'pants' THEN 'trousers'
            WHEN 'shirt' THEN 'top'
  ...
  scored AS (
    SELECT
      ...
      pa.pattrs,
      (SELECT qtok_raw FROM tokenize_q) AS qtok_raw,
  ...
  attr_calc AS (
    SELECT s.*,
      CASE
        ...
        ) / GREATEST(array_length(qtok_raw,1)::float8, 1)
      END AS attr_sim_v,
      CASE
        WHEN qkw IS NULL OR pattrs IS NULL THEN ARRAY[]::text[]
        ELSE ARRAY(SELECT unnest(qkw) INTERSECT SELECT unnest(pattrs) ORDER BY 1)
  ...
  SELECT id, item_name, item_name_en, vendor_name, category,
         ...
         LEAST(1.0, base_score_v + COALESCE(w_attr * attr_sim_v, 0)) AS final_score_v,
         used_signals_v,
         qtok_raw AS normalized_query_tokens,
         qkw AS synonym_expanded_tokens,
         COALESCE(pattrs, ARRAY[]::text[]) AS product_attr_tokens
  FROM weighted
  WHERE base_score_v IS NOT NULL
    AND LEAST(1.0, base_score_v + COALESCE(w_attr * attr_sim_v, 0)) >= match_threshold
  ORDER BY LEAST(1.0, base_score_v + COALESCE(w_attr * attr_sim_v, 0)) DESC
  LIMIT max_results;
$function$;