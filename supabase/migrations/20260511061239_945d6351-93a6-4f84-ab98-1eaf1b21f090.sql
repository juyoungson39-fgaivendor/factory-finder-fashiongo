...
BEGIN
  ...
  SELECT sp.factory_id, AVG(sp.embedding)::vector AS emb, MIN(f.user_id) AS factory_user_id
  ...
  WITH scored AS (
    SELECT ta.id AS trend_analysis_id, fe.factory_id,
           ...
           COALESCE(ta.user_id, fe.factory_user_id) AS uid
    FROM public.trend_analyses ta
    CROSS JOIN _factory_emb fe
    WHERE ta.embedding IS NOT NULL AND ta.status = 'analyzed'
  ),
  ins AS (
    INSERT INTO public.trend_matches (trend_analysis_id, factory_id, match_score, status, user_id)
    SELECT trend_analysis_id, factory_id, score, 'candidate', uid
    FROM scored WHERE score >= p_threshold AND uid IS NOT NULL
