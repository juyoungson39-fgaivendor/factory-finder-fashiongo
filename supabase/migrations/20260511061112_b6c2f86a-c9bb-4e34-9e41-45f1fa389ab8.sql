...
BEGIN
  ...
  SELECT
    sp.factory_id AS factory_id,
    AVG(sp.embedding)::vector AS emb
  FROM public.sourceable_products sp
  JOIN public.factories f
    ON f.id = sp.factory_id AND f.deleted_at IS NULL
  ...
  ins AS (
    INSERT INTO public.trend_matches
      (trend_analysis_id, factory_id, match_score, applied_threshold, signal_factors)
    SELECT
      trend_analysis_id, factory_id, score, p_threshold,
      jsonb_build_object('source','embedding_cosine','method','factory_avg_sourceable')
    FROM scored
    WHERE score >= p_threshold
    ON CONFLICT (trend_analysis_id, factory_id) DO UPDATE
      SET match_score = EXCLUDED.match_score,
          applied_threshold = EXCLUDED.applied_threshold,
          signal_factors = EXCLUDED.signal_factors
    RETURNING 1
  )
