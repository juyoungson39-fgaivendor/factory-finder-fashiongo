...
const SourceBadge = ({ type, query }: { type: string | null; query: string | null }) => {
  ...
  const label = type === 'image' ? '이미지 검색' : type === 'text' ? '텍스트 검색' : '트렌드';
  return (
...
const SourcingTargetFG = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, field, value }: { id: string; field: string; value: any }) => {
      const { error } = await (supabase as any).from('products').update({ [field]: value }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={() => setFilterSource(null)} className={`text-xs px-2 py-1 rounded transition-colors ${!filterSource ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>전체</button>
        {sourceCounts['image'] && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {filtered.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className={`rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden flex flex-col relative group ${isSelected ? 'ring-2 ring-primary' : ''}`}
            >
              {/* Checkbox + Delete */}
