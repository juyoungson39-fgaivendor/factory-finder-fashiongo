...
import { Plus, Trash2, GripVertical, Star, Sparkles, ChevronUp, Weight } from 'lucide-react';
...
const SortableCriteriaItem = ({
  ...
  const ts = tierStyles[t];
  const weightPct = maxWeight > 0 ? ((c.weight ?? 1) / maxWeight) * 100 : 0;

  return (
    <div ref={setNodeRef} style={style}>
      <Card className={`${ts.border} ${isDragging ? 'shadow-lg ring-2 ring-primary/20 opacity-90' : 'hover:shadow-sm'} transition-shadow`}>
        <CardContent className="flex items-center gap-4 py-3.5 px-4 group">
          ...
          <span className="text-[11px] text-muted-foreground/40 font-mono w-5 text-center shrink-0">{idx + 1}</span>
...
const ScoringSettings = () => {
  ...
  const grouped = {
    critical: criteria.filter((c) => getWeightTier(c.weight).tier === 'critical').length,
    important: criteria.filter((c) => getWeightTier(c.weight).tier === 'important').length,
    normal: criteria.filter((c) => getWeightTier(c.weight).tier === 'normal').length,
    minor: criteria.filter((c) => getWeightTier(c.weight).tier === 'minor').length,
  };
            ...
            {(['critical', 'important', 'normal', 'minor'] as const).map((tier) => {
              ...
              return (
                <span key={tier} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${tierStyles[tier].badge}`}>
                  {getWeightTier(tier === 'critical' ? 2.5 : tier === 'important' ? 1.5 : tier === 'normal' ? 1 : 0.5).label} {grouped[tier]}
                </span>
              );
            })}
          </div>
