...
const Dashboard = () => {
  ...
  const STEPS = ['트렌드 분석','공장 매칭','벤더 배분','상품 컨펌','정보 완성','FG 등록'];
      ...
      {(() => {
        ...
        const line = points.map((y, i) => `${i === 0 ? 'M' : 'L'}${(i * step).toFixed(1)},${(h - y).toFixed(1)}`).join(' ');
        return (
          <div style={{ background: '#ffffff', border: '1px solid #e1e3e5', borderRadius: 6, boxShadow: '0 1px 0 rgba(26,26,26,0.07)', marginBottom: 16, width: '100%' }}>
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ display: 'block' }}>
                <line x1="0" y1={h} x2={w} y2={h} stroke="#e1e3e5" strokeWidth="0.5" />
                <path d={line} fill="none" stroke="#008060" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
              </svg>
              {/* Legend */}
