...
const SourcingTargetOther = () => {
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState<SortKey>('newest');

  const { data: items = [], isLoading } = useQuery({
    queryKey: ['sourcing-targets', 'other'],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <ProductTable items={filtered} isLoading={isLoading} showSource emptyText="SNS/타 사이트에서 가져온 소싱 타깃 상품이 없습니다" />
    </div>
  );
};

export default SourcingTargetOther;
