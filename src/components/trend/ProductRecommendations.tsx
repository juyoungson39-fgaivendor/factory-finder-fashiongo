...
const ProductRecommendations = () => {
  ...
  return (
    <div className="space-y-5">
      {/* Score Formula */}
      <div className="rounded-xl border border-[#4f46e5]/20 bg-[#f5f3ff] p-3 text-xs text-[#4f46e5]">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="w-36">
          <label className="text-xs font-medium text-[#6d7175] mb-1 block">카테고리</label>
        ...
        <div className="w-40">
          <label className="text-xs font-medium text-[#6d7175] mb-1 block">정렬</label>
        ...
        <div className="w-48">
          <label className="text-xs font-medium text-[#6d7175] mb-1 block">최소 점수: {minScore}</label>
          <Slider value={[minScore]} onValueChange={v => setMinScore(v[0])} min={0} max={100} step={5} />
        </div>
        {selectedKeyword && (
          <button onClick={() => setSelectedKeyword(null)}
            className="text-xs text-[#4f46e5] underline self-end pb-2">
      ...
      {scoredProducts.length === 0 ? (
        <div className="text-center py-16 text-[#6d7175]">조건에 맞는 상품이 없습니다.</div>
      ) : (
