          ...
          <div className="w-full overflow-auto rounded-lg" style={{ border: '1px solid #e1e3e5' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        ...
                        <tr key={p.id} style={{ borderBottom: i < items.length - 1 ? '1px solid #e1e3e5' : 'none' }}
                                      ...
                                      <td style={{ padding: '8px 12px', width: 60 }}>
                                        {p.image_url ? (
                                                            ...
                                                            <div style={{ width: 50, height: 62, borderRadius: 4, border: '1px solid #e1e3e5', background: '#f6f6f7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                                                <span style={{ fontSize: 9, color: '#b5b5b5' }}>No img</span>span>
                                                            </div>div>
                                                      )}
                                      </td>td>
                                      <td style={{ padding: '10px 12px', minWidth: 140 }}>
                                                      <span style={{ fontSize: 13, fontWeight: 600, color: '#202223', fontFamily: 'monospace' }}>{p.product_no}</span>span>
                                      </td>td>
                                      <td style={{ padding: '10px 12px' }}>
                                                      <span style={{ fontSize: 12, background: '#f1f2f3', padding: '2px 8px', borderRadius: 4, color: '#202223' }}>{p.category ?? '—'}</span>span>
                                      </td>td>
                                      <td style={{ padding: '10px 12px', fontSize: 13, fontWeight: 600, color: '#202223', whiteSpace: 'nowrap' }}>
                                        {p.price != null ? `$${p.price.toFixed(2)}` : '—'}
                                      </td>td>
                                      <td style={{ padding: '10px 12px', fontSize: 12, color: '#6d7175', maxWidth: 150 }}>{p.material || '—'}</td>td>
                                      <td style={{ padding: '10px 12px', fontSize: 12, color: '#6d7175', maxWidth: 200 }}>{p.color_size || '—'}</td>td>
                          ...
                          {items.length === 0 && (
                        <tr>
                                      <td colSpan={headers.length} style={{ padding: 40, textAlign: 'center', color: '#6d7175', fontSize: 13 }}>{emptyText}</td>td>
                        </tr>tr>
                                  )}
                        </tbody>tbody>
                </table>table>
...
export default ProductTable;</div>
          )
    }
}
}
